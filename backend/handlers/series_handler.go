...
type SeriesHandler struct {
	...
	// Delegate single-part video generation to VideoHandler
	videoHandler *VideoHandler
}

// NewSeriesHandler creates a SeriesHandler sharing services from VideoHandler
func NewSeriesHandler(cfg *config.Config, vh *VideoHandler) *SeriesHandler {
	return &SeriesHandler{
		cfg:               cfg,
		geminiService:     vh.geminiService,
		audioService:      vh.audioService,
		stockVideoService: vh.stockVideoService,
		composerService:   vh.composerService,
		textProcessor:     vh.textProcessor,
...
func (sh *SeriesHandler) processSeriesGeneration(seriesID string, req models.SeriesGenerateRequest) {
	...
	// Clean up stock video tracking for child jobs
	for _, jobID := range func() []string {
		sh.seriesMu.RLock()
		defer sh.seriesMu.RUnlock()
		if s, ok := sh.series[seriesID]; ok {
			return s.ChildJobIDs
		}
		return nil
	}() {
		// jobID có thể rỗng nếu part đó failed script và chưa tạo child job
		if jobID != "" {
			sh.stockVideoService.CleanupJob(jobID)
		}
	}
}
...
func (sh *SeriesHandler) runPartGeneration(seriesID string, idx int) {
	...
	// Mint a real jobID and register it in VideoHandler's jobs map
	jobID := uuid.New().String()
	sh.seriesMu.Lock()
	job.ChildJobIDs[idx] = jobID
	sh.seriesMu.Unlock()

	// Register the job in VideoHandler
	partJob := sh.videoHandler.registerJob(jobID, genReq)
	...
	go func() {
		for {
			...
			case <-time.After(1 * time.Second):
				sh.videoHandler.jobsMux.RLock()
				vj, ok := sh.videoHandler.jobs[jobID]
				sh.videoHandler.jobsMux.RUnlock()
				if !ok {
					return
				}
	...
	sh.videoHandler.processVideoGenerationWithSegments(jobID, genReq, script, partJob)
	close(done)
	...
	sh.updateOverallStatus(seriesID)
	// Cleanup stock videos for this part's child job
	sh.stockVideoService.CleanupJob(jobID)
}
