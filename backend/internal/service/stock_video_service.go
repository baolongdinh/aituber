...
func (sv *StockVideoService) FetchSourceMaterial(ctx context.Context, keywords string, visualDesc string, t2vModel, t2vProvider string, jobID string, segIndex int, orientation string) (*StockMaterial, error) {
	if orientation == "" {
	...
	if visualDesc != "" {
		...
		if imgBytes, err := os.ReadFile(cachePath); err == nil {
			...
			return &StockMaterial{Type: "image", ImageBytes: imgBytes}, nil
		}
	}
	...
	if err == nil && len(infos) > 0 {
		mInfos := make([]VideoInfo, len(infos))
		for i, v := range infos {
			mInfos[i] = VideoInfo{Link: v.Link, Duration: v.Duration}
		}
		return &StockMaterial{Type: "pexels", PexelsInfo: mInfos}, nil
	}
...
func (sv *StockVideoService) PrepareVideoFromMaterial(ctx context.Context, material *StockMaterial, audioDuration float64, jobID string, segIndex int, orientation string) (string, error) {
	...
	case "pexels":
		...
		// Convert VideoInfo back to internal videoInfo for existing helpers
